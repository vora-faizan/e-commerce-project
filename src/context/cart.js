...
const CartProvider = ({ children }) => {
    const [cart, setCart] = useState(() => {
      ...
      const [clicked, setClicked] = useState(() => {
        // Try to get cart data from localStorage, if not found, return an empty array
        const storedCart = localStorage.getItem('heart');
        return storedCart ? JSON.parse(storedCart) : [];
      });
    console.log(cart);
    useEffect(() => {
    ...
    const handleClick = (heart) => {
        // Assuming clicked is initially an empty array in your state
        const productIdx = clicked.findIndex((item) => item.heart._id === heart.id);
        if (productIdx !== -1) {
            increaseQuantity(heart._id);
        } else {
            setClicked([...clicked, { heart, quantity: 1 }]);
        }
    };
